...
def collect_project_data(
    ...
    print(
        f"识别到 {len(project_data.nodes)} 个节点 "
    ...
    # 第三步：构建关系图
    project_data.import_edges, project_data.child_edges = _build_relationships(
        project_data.nodes
    )
    ...
    print(
        f"分析完成: {len(project_data.import_edges)} 个导入关系, "
...
def _walk_python_files(
    base_path: Path, include: List[str], exclude: List[str]
) -> List[Path]:
    """遍历目录收集Python文件"""
    python_files = []
...
def _should_include_file(
    file_path: Path, base_path: Path, include: List[str], exclude: List[str]
) -> bool:
    """检查文件是否应被包含"""
    try:
...
def _identify_nodes(
    all_files: Dict[str, Path], root_paths: List[str]
) -> Dict[str, NodeInfo]:
    ...
    for package_name, init_file in package_dirs:
        nodes[package_name] = NodeInfo(
            name=package_name, node_type=NodeType.PACKAGE, file_path=str(init_file)
        )
    ...
    for module_name, file_path in all_files.items():
        if file_path.name != "__init__.py":
            nodes[module_name] = NodeInfo(
                name=module_name, node_type=NodeType.MODULE, file_path=str(file_path)
            )
...
def _analyze_node_imports(node: NodeInfo, all_nodes: Dict[str, NodeInfo]) -> None:
    ...
    class ImportVisitor(ast.NodeVisitor):
        ...
        def visit_ImportFrom(self, node_ast: ast.ImportFrom) -> None:
            ...
            container_parts = (
                curr_parts[:-1] if (is_module and len(curr_parts) > 1) else curr_parts
            )
            ...
            if level <= 0:
                ...
                base_parts = (
                    container_parts[: -(level - 1)]
                    if (level - 1) <= len(container_parts)
                    else []
                )
            ...
            if node_ast.module:
                ...
                if level == 0:
                    ...
                    for alias in node_ast.names:
                        if alias.name != "*":
                            full_name = ".".join(
                                [p for p in (*base_parts, *tail_parts, alias.name) if p]
                            )
                            if full_name:
