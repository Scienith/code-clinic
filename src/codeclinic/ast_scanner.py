...
def scan_project_ast(
    paths: List[str], include: List[str], exclude: List[str], count_private: bool
) -> Tuple[Modules, GraphEdges, ChildEdges, StubFunctions]:
    """
...
def _resolve_import(
    imported_name: str, importer_module: str, all_modules: Dict[str, Path]
) -> Optional[str]:
        ...
        if len(parts) > 1:
            ...
            if stripped_top in internal_toplevel:
                for module_name in all_modules:
                    if module_name == stripped or module_name.startswith(
                        stripped + "."
                    ):
                        return module_name
        return None
...
def _count_functions_and_stubs(
    file_path: Path, module_name: str, count_private: bool
) -> Tuple[ModuleStats, StubFunctions]:
