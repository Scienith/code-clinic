...
def show_config() -> None:
    ...
    try:
        ...
        print(f"  ⛔ forbid_private_modules: {'开启' if getattr(rules, 'forbid_private_modules', False) else '关闭'}")
...
def format_config_display(config: ExtendedConfig) -> str:
    ...
    lines.append(f"  ⛔ forbid_private_modules: {'开启' if getattr(rules, 'forbid_private_modules', False) else '关闭'}")
