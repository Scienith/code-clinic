...
from .node_types import FunctionInfo, NodeInfo, ProjectData
...
def analyze_stub_completeness(project_data: ProjectData) -> Dict[str, Any]:
    ...
    result = {"stub_functions": stub_functions}
    # 输出简单统计（不再报告项目级占比）
...
def save_stub_report(
    stub_data: Dict[str, Any], project_data: ProjectData, output_dir: Path
) -> Path:
    """
    保存Stub完整度报告
...
def _prepare_stub_json_data(
    stub_data: Dict[str, Any], project_data: ProjectData
) -> Dict[str, Any]:
...
def _generate_stub_recommendations(stub_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 高优先级节点建议
    high_priority = [
        node for node in stub_data["node_stats"] if node["priority_score"] > 60
    ]
    if high_priority:
        recommendations.append(
            {
...
def _generate_stub_heatmap(
    ...
    try:
        ...
        if show_test_borders:
            ...
            candidates = [
                output_dir / "component_tests.json",
                output_dir.parent / "component_tests.json",
            ]
