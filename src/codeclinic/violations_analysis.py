...
from .node_types import ImportViolation, ProjectData
...
def save_violations_report(
    violations_data: Dict[str, Any], project_data: ProjectData, output_dir: Path
) -> Path:
    """
    保存违规分析报告
...
def _prepare_json_data(
    violations_data: Dict[str, Any], project_data: ProjectData
) -> Dict[str, Any]:
        ...
        "violations": [
            {
                ...
                "message": v.message,
                "from_node_type": (
                    project_data.nodes.get(v.from_node, {}).node_type.value
                    if project_data.nodes.get(v.from_node)
                    else "unknown"
                ),
                "to_node_type": (
                    project_data.nodes.get(v.to_node, {}).node_type.value
                    if project_data.nodes.get(v.to_node)
                    else "unknown"
                ),
            }
...
def _extract_rules_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if hasattr(import_rules, "__dict__"):
        ...
        return {
            ...
            "forbid_private_modules": getattr(
                import_rules, "forbid_private_modules", False
            ),
        }
        ...
        return {
            ...
            "allow_patterns": import_rules.get("allow_patterns")
            or import_rules.get("allowed_patterns"),
            "deny_patterns": import_rules.get("deny_patterns")
            or import_rules.get("denied_patterns"),
...
def _calculate_node_violation_stats(
    ...
    for violation in violations:
        ...
        if violation.from_node not in from_violations:
            from_violations[violation.from_node] = {"total": 0, "by_type": {}}
        ...
        if violation.to_node not in to_violations:
            to_violations[violation.to_node] = {"total": 0, "by_type": {}}
...
def _generate_violations_graph(
    ...
    try:
        ...
        legal = (
            set(violations_data["legal_edges"])
            if isinstance(violations_data.get("legal_edges"), set)
            else set(violations_data.get("legal_edges", []))
        )
        viol = (
            set(violations_data["violation_edges"])
            if isinstance(violations_data.get("violation_edges"), set)
            else set(violations_data.get("violation_edges", []))
        )
        ...
        try:
            ...
            for name, node in project_data.nodes.items():
                if (
                    ...
                    and node.node_type.value == "package"
                ):
                    for imp in getattr(node, "imports", set()):
                        if imp in project_data.nodes and (
                            imp.startswith(name + ".") or imp == name
                        ):
        ...
        # 优先展示违规边：如同一对 (src, dst) 既在合法集又在违规集，移出合法集避免双重着色
        if legal and viol:
            legal = {e for e in legal if e not in viol}
